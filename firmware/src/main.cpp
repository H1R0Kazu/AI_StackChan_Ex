...
volatile bool servo_random_active = false;
volatile int servo_rand_x = 0;
volatile int servo_rand_y = 0;
...
void servo(void *args)
{
  ...
  // 状態管理: 同じ位置への重複コマンドを避けるため、エッジトリガーで動作
  enum ServoState { ST_HOME, ST_GAZE_AVATAR, ST_GAZE_RANDOM };
  ServoState prev_state = ST_HOME;
  int prev_rand_x = 0, prev_rand_y = 0;
  int prev_gaze_x = 0, prev_gaze_y = 0;
  ...
  for (;;)
  {
    ...
    if (!servo_home) {
      current_state = ST_GAZE_AVATAR;
    } else if (servo_random_active) {
      current_state = ST_GAZE_RANDOM;
    } else {
      current_state = ST_HOME;
    }

    if (current_state == ST_GAZE_AVATAR) {
      // アバター追従: 視線ターゲットが変わった時のみコマンド発行
      avatar->getGaze(&gazeY, &gazeX);
      int deg_x = (int)(15.0 * gazeX);
      int deg_y = (int)(10.0 * gazeY);
      if (prev_state != ST_GAZE_AVATAR || prev_gaze_x != deg_x || prev_gaze_y != deg_y) {
        robot->servo->moveTo(deg_x, deg_y);
        prev_gaze_x = deg_x;
        prev_gaze_y = deg_y;
      }
    } else if (current_state == ST_GAZE_RANDOM) {
      // ランダム動作: 状態遷移時 or 目標値変化時のみコマンド発行
      if (prev_state != ST_GAZE_RANDOM || prev_rand_x != servo_rand_x || prev_rand_y != servo_rand_y) {
        robot->servo->moveTo(servo_rand_x, servo_rand_y);
        prev_rand_x = servo_rand_x;
        prev_rand_y = servo_rand_y;
      }
    } else {
      // ホーム位置: 状態遷移時のみコマンド発行
      if (prev_state != ST_HOME) {
        robot->servo->moveToOrigin();
      }
    }
    prev_state = current_state;

    // サーボ健全性チェック: 過負荷等でトルクが落ちた場合に自動復旧する
    {
      static uint32_t healthCheckTime = 0;
      if (millis() - healthCheckTime > 3000) {
        for (uint8_t id = AXIS_X + 1; id <= AXIS_Y + 1; id++) {
          if (robot->servo->readTorqueEnable(id) == 0) {
            bool ok = robot->servo->ensureTorqueOn(id);
            Serial.printf("[ServoHealth] torque recovery id=%d -> %s\n", id, ok ? "OK" : "FAIL");
          }
        }
...
void setup()
{
  ...
  // RT_DYN_XL330はDynamixel応答読み取り+浮動小数点printfでスタック消費が大きいため2048では不足
